...
func isBlocked(targetURL string) bool {
	...
	for _, blocked := range blackList {
		if strings.HasPrefix(targetURL, blocked) || strings.HasPrefix(host, blocked) || strings.HasSuffix(host, "."+blocked) {
			return true
		}
	}
...
func proxyHandler(w http.ResponseWriter, r *http.Request) {
	...
	if isBlocked(targetURL) {
		...
		fmt.Fprintf(w, "Access to %s is blocked by proxy blacklist\n", targetURL)
		writeLog(targetURL, http.StatusForbidden, "(blocked)")
		return
	}
...
func main() {
	...
	if err := loadBlacklist(configPath); err != nil {
		panic(err)
	}
