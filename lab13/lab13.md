...
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   ...
   - 0x6e76fc54. Значения одинаковые у Request и ACK — это случайное число, которое клиент придумывает в начале сессии, чтобы сопоставить свой запрос с ответом сервера.
   
<img src="images/dhcp1.png" width=700 />
...
4. Хост использует DHCP-протокол, в частности, для получения IP-адреса. Но IP-адрес хоста
   ...
   - Пока адрес ещё не получен, клиент шлёт с 0.0.0.0 на 255.255.255.255 (широковещательно), так как своего IP у него ещё нет, а сервер ему неизвестен.
5. Каков IP-адрес вашего DHCP-сервера?
   - 192.168.3.1
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   вашем эксперименте?
   - Срок аренды — это время, на которое сервер выдаёт клиенту IP-адрес, после чего клиент должен его обновить или адрес освобождается. У меня IP Address Lease Time: 1 day

<img src="images/dhcp2.png" width=700 />
