...
def checksum_data(seq, eof, payload):
    header = json.dumps({"seq": seq, "eof": eof}, sort_keys=True).encode()
...
def make_frame(seq, payload, eof=False):
    header = json.dumps({
        "seq": seq,
        "eof": eof,
        "checksum": checksum(checksum_data(seq, eof, payload)),
    }).encode()
    return header + b"\n\n" + payload
...
def parse_frame(data):
    ...
    frame = json.loads(header.decode())
    expected = frame["checksum"]
    actual = checksum(checksum_data(frame["seq"], frame.get("eof", False), payload))
    print(f"server: checksum received={expected}, calculated={actual}")
    if actual != expected:
