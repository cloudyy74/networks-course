...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - как видно из скрина: информация о браузере (User-Agent), какой формат браузер может принять (Accept), какие алгоритмы сжатия он понимает (Accept-Encoding) и т.д.
...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - да
   - статус код 200, в теле ответа явно лежит содержимое файла
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону
   ...
   - If-Modified-Since: Tue, 28 Oct 2025 05:59:01 GMT $-$ время последнего изменения файла, которое браузер закэшировал
4. Какой код состояния HTTP и фраза возвращаются сервером в ответ на этот второй запрос
   ...
   - 304 Not Modified
   - нет, сервер не возвращает содержимое файла, браузер берет его из кэша
...
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - параллельно. нетрудно видеть, что второй запрос был отправлен до получения ответа на первый. браузер просто открыл два tcp-соединения на эти запросы (это можно видеть по разным src портам, например)
...
2. Когда ваш браузер отправляет сообщение HTTP GET во второй раз, какое новое поле включается в сообщение HTTP GET?
   - Authorization c токеном и Cache-Control: max-age=0, чтобы не кэшировать токен
